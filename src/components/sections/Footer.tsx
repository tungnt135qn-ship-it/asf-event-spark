    ...
    <footer className="border-t border-white/10 bg-navy-deep/80 px-4 pt-16 pb-8 backdrop-blur-md">
      <div className="mx-auto max-w-7xl">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-white/70">
              <li className="flex items-start gap-2">
                ...
                <span>{info?.address || t("footer.location")}</span>
              </li>
              {info?.email || true ? (
                <li className="flex items-start gap-2">
                  <Mail size={16} className="mt-0.5 shrink-0 text-gold" />
                  <a href={`mailto:${info?.email || "asf2026@vbma.org.vn"}`} className="hover:text-gold">
                    {info?.email || "asf2026@vbma.org.vn"}
                  </a>
                </li>
              ) : null}
              {(info?.phone || true) && (
                <li className="flex items-start gap-2">
                  ...
                  <span>{info?.phone || "+84 24 xxxx xxxx"}</span>
                </li>
              )}
            </ul>
          </div>
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-6 text-xs text-white/50 sm:flex-row">
          <div>{footerText || t("footer.rights")}</div>
