...
export function Library() {
  ...
  return (
    <Section id="library" eyebrow="Library" title="Photos, Videos & Documents">
      {/* Filters */}
      ...
      {/* Documents block (re-grouped under Library) */}
      <div className="mt-12">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-bold text-white">Documents</h3>
          <a href="#documents" className="text-xs font-semibold text-gold hover:underline">
            View all →
          </a>
        </div>
        <p className="text-sm text-white/60">
          Official brochures, agendas, reports and travel guides — see the Documents section below.
        </p>
      </div>
      ...
      {preview && (
        ...
        >
          ...
          <div onClick={(e) => e.stopPropagation()} className="max-h-[85vh] w-full max-w-5xl">
            {preview.type === "photo" ? (
              <img src={preview.src} alt={preview.title} className="mx-auto max-h-[85vh] rounded-xl object-contain" />
            ) : (
              <video src={preview.src} controls autoPlay className="mx-auto max-h-[85vh] w-full rounded-xl bg-black" />
            )}
            <div className="mt-3 text-center text-sm text-white/80">
              {preview.title} · Day {preview.dayIndex}
            </div>
          </div>
        </div>
      )}
